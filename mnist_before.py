...
def main():
    ...
    for _ in range(args.n_runs):
        ...
        run_result = test(args, model, device, test_loader)

        if args.save_model:
            if args.disable_dp:
                save_name = "{}/MNIST_reference_{}.pth".format(args.dataset, _)
                torch.save(model.state_dict(), save_name)
                with open(save_name.replace('.pth','.txt'), "a", encoding="utf-8") as f:
                    f.write(str(args) + '\nACC: ' + str(run_result))    
            else:
                save_name = "{}/MNIST_dp.pth".format(args.dataset)
                torch.save(model.state_dict(), save_name)
                with open(save_name.replace('.pth','.txt'), "a", encoding="utf-8") as f:
                    f.write(str(args) + '\nACC: ' + str(run_result))     
